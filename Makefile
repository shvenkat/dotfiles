...
#    configured.
#
# Q. Are there any software requirements to using this Makefile?
# A. A *nix environment, make and bash are assumed to be present. Other basic
#    software requirements for this Makefile - such as curl, git and python -
#    are kept to a minimum and declared as prerequisites for the relevant
#    targets. Their installation _may_ be left to the user, in which case a
#    message may be printed asking the user to install them.
#
...
PKG_MANAGER := brew
PKG_INSTALL := install
PKG_CHECK := list --versions
PKG_UPDATE := upgrade
PKG_REMOVE := uninstall
else
...
help:
	...
	echo "    make dotfiles-update    Install dotfiles."
	...
	echo "DESCRIPTION"
	echo
	echo "    Installs and/or updates various config and application files."
	echo
	...
	$(foreach target,$(EXTENSIONS),echo "        $(target)";)
...
.PHONY: brew ruby python3
.SILENT: brew ruby python3

prereq: brew ruby python3
...
.PHONY: dotfiles dotfiles-update dotfiles-clean $(DOTFILES) $(DOTFILES_UPDATE)

dotfiles: $(DOTFILES)

dotfiles-update: $(DOTFILES_UPDATE)
...
# cask "shvenkat/extra/isabelle2014"
